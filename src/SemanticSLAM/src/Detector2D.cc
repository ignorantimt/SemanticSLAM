...
namespace ORB_SLAM2 {
    ...
    dynamic_detection_confidence_threshold(dynamic_detection_confidence_threshold_)
    {
        ...
        detect_net_ptr->load_param("/home/rob/Robotics/SemanticSLAM/src/SemanticSLAM/Thirdparty/ncnn_model/mobilenetv3_ssdlite_voc.param");
        detect_net_ptr->load_model("/home/rob/Robotics/SemanticSLAM/src/SemanticSLAM/Thirdparty/ncnn_model/mobilenetv3_ssdlite_voc.bin");
    ...
    void Detector2D::draw_objects(cv::Mat &image)
    {
        for (size_t i = 0; i < mvObjects2D_to_View.size(); i++)
        {
            ...
            if(obj2d.prob > detection_confidence_threshold)
            {
                ...
                char text[256];
                sprintf(text, "%s, prob: %.1f%%", obj2d.name.c_str(), obj2d.prob * 100);
