...
{
public:
    ORBSlamNode() : Node("rgbd_tum")
    {
        CamPose_Pub = this->create_publisher<geometry_msgs::msg::PoseStamped>("/Camera_Pose", 10);
        Camodom_Pub = this->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>("/Camera_Odom", 10);
...
void ORBSlamNode::Pub_CamPose(cv::Mat &pose)
{
    ...
    else
    {
        ...
        transformStamped.transform.translation.y = -twc.at<float>(0);
        transformStamped.transform.translation.z = -twc.at<float>(1);
        ...
        cam_pose.pose.position.x = transformStamped.transform.translation.x;
        cam_pose.pose.position.y = transformStamped.transform.translation.y;
        cam_pose.pose.position.z = transformStamped.transform.translation.z;
        cam_pose.pose.orientation = transformStamped.transform.rotation;

        CamPose_Pub->publish(cam_pose);
    }
}

void ORBSlamNode::LoadImages(const string &strAssociationFilename, vector<string> &vstrImageFilenamesRGB,
                             vector<string> &vstrImageFilenamesD, vector<double> &vTimestamps)
{
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<ORBSlamNode>();
    ...
    node->LoadImages(strAssociationFilename, vstrImageFilenamesRGB, vstrImageFilenamesD, vTimestamps);
    ...
    for (int ni = 0; ni < nImages && rclcpp::ok(); ni++)
    {
        ...
        cv::Mat Camera_Pose = SLAM.TrackRGBD(imRGB, imD, tframe);
